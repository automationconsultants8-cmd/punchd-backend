...
export class FaceRecognitionService {
  ...
  // If no reference photo, use this clock-in photo as the reference
  if (!user?.referencePhotoUrl) {
    console.log(`No reference photo for user ${userId}, storing first clock-in photo as reference`);
    await this.prisma.user.update({
      where: { id: userId },
      data: { referencePhotoUrl: clockInPhotoUrl },
    });
    return {
      matched: true,
      confidence: 100,
      skipped: false,
      reason: 'First clock-in - reference photo stored',
    };
  }
